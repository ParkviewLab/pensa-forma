...
One small domain carried all the way through: its record in the on-disk canonical form, the layout the [layout engine](layout-engine.md) computes for it, and the drawing the [mark geometry](mark-geometry.md) makes of that layout, which the HTML sibling of this file renders. It is a fixture: an implementation that produces these numbers from this record, and this picture from these numbers, has the layout and the marks right. It also shows, on one screen, how a domain reads: two branches leaving one point on the left and returning to one point above, a project inside the main workflow and another inside a branch, the openers and closers of every workflow, and a diamond at every point where a branch may depart or arrive. The file is generated by `scripts/worked_example.py` from the rules; when a rule changes, the script changes and the example is regenerated, so the two cannot drift.
